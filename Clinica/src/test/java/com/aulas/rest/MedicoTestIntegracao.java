...
public class MedicoTestIntegracao {
	...
	void deveRetornarOkQuandoPesquisaIdExistente() throws Exception {
		ResultActions result = mockMvc
				.perform(get("/medico/{idmedico}", idExistente).accept(MediaType.APPLICATION_JSON));
		result.andExpect(status().isOk());
	}

	@Test
	void deveRertornarStatus404QuandoPesquisaIdInexistente() throws Exception {
		ResultActions obj = mockMvc.perform(get("/{idmedico}", idInexistente).accept(MediaType.APPLICATION_JSON));
	...
	void deveRetornarStatus404QuandoAlterarMedicoInexistente() throws Exception {
		...
		ResultActions result = mockMvc.perform(put("/{idmedico}", idInexistente).content(jsonBody)
	...
	void deveRetornarStatu201QuandoContatoSalvoComSucesso() throws Exception {
		...
		String jsonBody = objectMapper.writeValueAsString(medicoNovo);
		ResultActions result = mockMvc.perform(post("/medico").content(jsonBody).contentType(MediaType.APPLICATION_JSON)
	...
	void deveRetornarStatus200QuandoAlterarContatoExistenteComSucesso() throws Exception {
		...
		ResultActions result = mockMvc.perform(put("/medico/{idmedico}", idExistente).content(jsonBody)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON));
